...
原文中的链接、强调和正文中的行内代码会合并进所属 Translation Unit，保留命令、标识符及其周围的空格。译文会插入到对应原文之后，形成 Bilingual Content。直接选择一个 `<code>` 时，只翻译选中元素里的文字。
...
- 更大 Region 内部的 `<pre>`、以块级方式显示的 `<code>`，以及只含代码的独立文本块；
